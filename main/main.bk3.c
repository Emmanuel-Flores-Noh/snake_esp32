...
void IRAM_ATTR renderizar(void *params) {
	...
	while(true) {
		...
		if(estadoJuego == MENU) {
			...
			swprintf(texto, 50, u"puntuacion: %i", contadorSecciones);
			hagl_put_text(display, texto, display -> width / 2 - 30, display -> height / 2, BLANCO, font6x9);
		}
		else if(estadoJuego == GANAR) {
			hagl_put_text(display, u"GANASTE", display -> width / 2 - 30, display -> height / 2 - 18, BLANCO, font6x9);
		}
		hagl_flush(display);
...
void app_main(void)
{
	...
	// rotarBitmap(bitmapCabezaArray, DERECHA, bitmapTemporalArray);
